...
async function generate(pagesDir: string, outFile: string): Promise<void> {
  ...
  for (const [i, entry] of entries.entries()) {
    ...
    // Registry must contain the WRAPPED island (expr), not the raw page (pageId)
    // so that renderHydratable can find the island in the registry
    registryLines.push(
      `  ${JSON.stringify(entry.name)}: ${expr}` + (i < entries.length - 1 ? "," : ""),
    );
...
async function generateTypes(outFile: string): Promise<void> {
  ...
  const types = [
    `// @generated by @ilha/router — do not edit`,
    ``,
    `/// <reference types="ilha" />`,
    `/// <reference types="@ilha/router" />`,
    ``,
    `import type { Island } from "ilha";`,
    ...
    ``,
    `// Virtual module: "ilha:pages"`,
    `declare module "ilha:pages" {`,
    `  export { pageRouter };`,
    `}`,
    ``,
    `// Virtual module: "ilha:registry"`,
